...
WITH cte1 AS ( SELECT [invoice_number], [invc_itm_nbr], fiscal_variant, doc_cal_month, COUNT(*) AS duplicate_count FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd WHERE ref_invc_nbr != '' AND invc_type IN ('zrk1', 'zrk2') AND sold_by IN ('10', '20', '30', '40') AND sales_org IN ('1000', '2000', '1200') AND CAST(created_date AS hist_cust_po_date) BETWEEN DATEADD(day, -180, GETDATE()) AND GETDATE() GROUP BY [invoice_number], [invc_itm_nbr], fiscal_variant, doc_cal_month ) SELECT 'BILL_NUM || BILL_ITEM || FISCVARNT || CALMONTH' AS PRIMARY_KEY_NAME, CAST(invoice_number AS VARCHAR) + ' || ' + CAST(invc_itm_nbr AS VARCHAR) + ' || ' + CAST(fiscal_variant AS VARCHAR) + ' || ' + CAST(doc_cal_month AS VARCHAR) AS PRIMARY_KEY_VALUE, 'BILL_NUM || BILL_ITEM || FISCVARNT || CALMONTH' AS ATTRIBUTE_NAME, CAST(invoice_number AS VARCHAR) + ' || ' + CAST(invc_itm_nbr AS VARCHAR) + ' || ' + CAST(fiscal_variant AS VARCHAR) + ' || ' + CAST(doc_cal_month AS VARCHAR) AS ATTRIBUTE_VALUE, 'duplicate_count' AS ADDITIONAL_COLUMN_NAME, duplicate_count AS ADDITIONAL_COLUMN_VALUE FROM cte1;

SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, HEADER_EAR_DTTM_LOCAL, '49' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);

SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, CUST_ORD_NBR, '50' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);

SELECT Distinct Descoped invoice_number&invc_itm_nbr, SOLD_BY,'18' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') and  invc_type in ('zf2','zf2d','zf2f');

select distinct 
LTRIM(RTRIM(isnull(bp.object_row_id,'0')))AS EBP_ID,
...
'1115' as MAPPING_ID
from adh_genpro_use2_prd.s_suppliers.business_partner_hhd bp  tablesample(10 percentage) with(nolock)
join adh_genpro_use2_prd.s_reference_data.emdm_c_bp_local_hhd bl with(nolock) on bl.ebp_id=bp.object_row_id
join adh_genpro_use2_prd.s_reference_data.c_bp_address_hhd A with(nolock) on A.ebp_id=bp.object_row_id
where bp.HUB_STATE_IND =1 and bl.HUB_STATE_IND =1 and A.HUB_STATE_IND =1
and bp.org_indv_flg='O';

select distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, Manifest_EAR_DTTM_local, '34' as MAPPING_ID from adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) where (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL) and HEADER_EAR_DTTM_local &gt;= dateadd(d, -90, CAST(Getdate() AS hist_cust_po_date)) and MANIFEST_EAR_DTTM_local &gt;= dateadd(d, -90, CAST(Getdate() AS hist_cust_po_date));
...
SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, acct_nbr, '53' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);

select distinct CONCAT(aa.consolidated_attrib,',',aa.invoice_number,',',aa.prod_id,',',aa.acct_nbr) as Primary_Key,aa.consolidated_attrib as Ontime_CONS_ATTRIBUTE from  adh_genpro_use2_prd.g_product360.ontime_delivery_hhd as aa tablesample(10 percentage) WITH (NOLOCK) left join adh_genpro_use2_prd.s_transportation.emanifest_data_hhd as em  WITH (NOLOCK) on aa.consolidated_attrib= em.consolidated_attrib left join adh_genpro_use2_prd.s_transportation.event_stop_header_hhd as ev WITH (NOLOCK) on aa.consolidated_attrib= ev.consolidated_attrib where (aa.consolidated_attrib IS NOT NULL OR aa.invoice_number IS NOT NULL OR aa.prod_id IS NOT NULL OR aa.acct_nbr IS NOT NULL);

select distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, Manifest_EAR_DTTM_local, '34' as MAPPING_ID from adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) where (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL) and HEADER_EAR_DTTM_local &gt;= dateadd(d, -90, CAST(Getdate() AS hist_cust_po_date)) and MANIFEST_EAR_DTTM_local &gt;= dateadd(d, -90, CAST(Getdate() AS hist_cust_po_date));

SELECT Distinct Descoped invoice_number&invc_itm_nbr,hist_ar_plant_key,'14' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and hist_ar_plant_key is not null AND LTRIM(RTRIM(hist_ar_plant_key))!='';

SELECT 'DOC_NUMBER_ITEM_KEY' AS PRIMARY_KEY_NAME, SO.col_header as per mapping AS PRIMARY_KEY_VALUE, 'DOC_NUMBER_ITEM_KEY ' AS ATTRIBUTE_NAME, SO.col_header as per mapping AS ATTRIBUTE_VALUE, 'ORD_NBR' + '<>' + 'ORD_LINE_NBR' AS ADDITIONAL_COLUMN_NAME, CONCAT_WS('<>', SO.order_nbr, SO.order_line_nbr) AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.g_order360.order_transaction_hhd SO WHERE SO.MSG_CD IN ('ND', 'UM', 'NS', 'AM', 'NW', 'BO', 'TR') AND SO.ship_plant != '015' AND SO.order_nbr IS NOT NULL AND SO.SOLD_BY = '10' AND SO.created_date BETWEEN DATEADD(D, -180, GETDATE()) AND GETDATE();

SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, prod_id AS ATTRIBUTE_VALUE, '39' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL) and prod_id IS NOT NULL and  ltrim(rtrim(prod_id))!='';
...
SELECT   territory_id, end_date_c,'Name<>Account_Name__c<>Account__c<>Affiliation_Type__c<>IsDeleted' as 'Additional_Column_Name',Concat_ws  ('<>',territory_name,account_name_c,acct__c,affiliation_type_c,is_del) as 'ADDITIONAL_COLUMN_VALUE','1100' as Mapping_id FROM adh_genpro_use2_prd.s_customers.account_affiliation_hhd WITH (NOLOCK) WHERE affiliation_type_c = 'EBP';

SELECT DISTINCT cust_nbr AS PRIMARY_KEY_1,ncdp_nbr AS COLUMN_1, '1352' as MAPPING_ID FROM adh_genpro_use2_prd.s_customers.general_customer_hhd with(nolock);

SELECT distinct t1.cust_nbr as primary_key, t1.vendor_name_1 as DQ_Column1, t2.vendor_name_1 as DQ_Column2,'89' AS MAPPING_ID FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd t1 tablesample(10 percentage) WITH (NOLOCK) LEFT JOIN adh_genpro_use2_prd.s_customers.general_customer_hhd t2 WITH (NOLOCK) ON t1.cust_nbr = t2.cust_nbr;
...
SELECT Distinct Descoped invoice_number&invc_itm_nbr, invoice_number,'17' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') and  invc_type in ('zf2','zf2d','zf2f');

select Distinct sb.Descoped invoice_number&invc_itm_nbr,sb.sold_to_party,so.cust_nbr, '1' AS MAPPING_ID from adh_genpro_use2_prd.g_order360.billing_transactions_hhd sb Left join adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd so on trim(sb.sold_to_party)=trim(so.cust_nbr) where sb.Invc_Date >= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) and sb.sold_by in ('10','20','30','40') and sb.sales_organization in ('1000','1200','2000') AND sold_to_party != 'ICC0033';

select distinct trim(sb.Descoped invoice_number&invc_itm_nbr) AS Descoped invoice_number&invc_itm_nbr,trim(sb.sold_to_party) AS Sold_To_billng,trim(sb.order_nbr) AS Ord_Nbr_billing,trim(sb.order_line_nbr) AS Ord_Line_Nbr_billing, trim(so.sold_to_party) AS Sold_To_order,trim(so.order_nbr) AS Ord_Nbr_order,trim(so.order_line_nbr) AS Ord_Line_Nbr_order, '3' as MAPPING_ID from adh_genpro_use2_prd.g_order360.billing_transactions_hhd sb tablesample(10 percentage) WITH (NOLOCK) left join adh_genpro_use2_prd.g_order360.order_transaction_hhd so tablesample(100 percentage)  WITH (NOLOCK) on trim(sb.sold_to_party)=trim(so.sold_to_party) and trim(sb.order_nbr)=trim(so.order_nbr) and trim(sb.order_line_nbr)=trim(so.order_line_nbr) where sb.Invc_Date &gt;= dateadd(d, -365 , CAST(Getdate() AS hist_cust_po_date)) and sb.sold_by in ('10','20','30','40') and sb.sales_organization in ('1000','1200','2000');
...
'1114' as MAPPING_ID
from adh_genpro_use2_prd.s_reference_data.c_bp_address_hhd A with(nolock) JOIN adh_genpro_use2_prd.s_suppliers.business_partner_hhd E with(nolock)
...
SELECT Distinct Descoped invoice_number&invc_itm_nbr, sales_organization,'19' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') and  invc_type in ('zf2','zf2d','zf2f');

SELECT Distinct Descoped invoice_number&invc_itm_nbr, trailer_code,'6' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and trailer_code is not null AND LTRIM(RTRIM(trailer_code))!='';

SELECT Distinct Descoped invoice_number&invc_itm_nbr, ship_plant,'7' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and ship_plant is not null AND LTRIM(RTRIM(ship_plant))!='';
...
, '905' AS MAPPING_ID
FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd with (nolock)
...
SELECT DISTINCT cust_nbr AS PRIMARY_KEY_1, bus_prtnr_type AS COLUMN_1, '1386' as MAPPING_ID FROM adh_genpro_use2_prd.s_customers.cust_master_sales_data_hhd with(nolock);

SELECT DISTINCT cust_nbr AS PRIMARY_KEY_1, bus_prtnr_type AS COLUMN_1, '1386' as MAPPING_ID FROM adh_genpro_use2_prd.s_customers.cust_master_sales_data_hhd with(nolock);

SELECT DISTINCT cust_nbr AS PRIMARY_KEY_1,ncdp_nbr AS COLUMN_1, '1351' as MAPPING_ID FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd with(nolock);

select distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) AS PRIMARY_KEY_VALUE,Delta_from_manifest,'59' as MAPPING_ID from adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) with (nolock) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);

SELECT Distinct Descoped invoice_number&invc_itm_nbr,invc_type,'12' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and invc_type is not null AND LTRIM(RTRIM(invc_type))!='';

SELECT Distinct Descoped invoice_number&invc_itm_nbr,hist_ar_plant_key,'14' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and hist_ar_plant_key is not null AND LTRIM(RTRIM(hist_ar_plant_key))!='';

SELECT Distinct Descoped invoice_number&invc_itm_nbr, invc_itm_nbr,'23' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') and  invc_type in ('zf2','zf2d','zf2f');

SELECT Distinct Descoped invoice_number&invc_itm_nbr, invc_created_date,'24' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') and  invc_type in ('zf2','zf2d','zf2f');

SELECT Distinct Descoped invoice_number&invc_itm_nbr,invc_type,'12' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and invc_type is not null AND LTRIM(RTRIM(invc_type))!='';

SELECT distinct Descoped invoice_number&invc_itm_nbr,order_alt_serve_flag,ship_plant,hist_ar_plant_key,COALESCE(ship_plant,0) as SHIP_PLANT_null, COALESCE(hist_ar_plant_key,0) as HIST_AR_PLANT_CD_null, LENG,'31' AS MAPPING_ID FROM (SELECT Descoped invoice_number&invc_itm_nbr,ship_plant,hist_ar_plant_key,CASE WHEN LEN(order_alt_serve_flag)=0 THEN 'B' WHEN order_alt_serve_flag IS NULL THEN 'N' ELSE order_alt_serve_flag END  AS order_alt_serve_flag,case when LEN(order_alt_serve_flag) is null then 0 else LEN(order_alt_serve_flag) end as LENG FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) where adh_genpro_use2_prd.g_order360.billing_transactions_hhd. Invc_Date &gt;= dateadd(d, -365, CAST(Getdate() AS hist_cust_po_date))) A;

SELECT Distinct Descoped invoice_number&invc_itm_nbr, hist_cust_bus_chnl_nbr,'13' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and hist_cust_bus_chnl_nbr is not null AND LTRIM(RTRIM(hist_cust_bus_chnl_nbr))!='';
...
SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, cust_ship_to_zip, '40' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL) and cust_ship_to_zip IS NOT NULL and ltrim(rtrim(cust_ship_to_zip))!='';

select distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, Manifest_EAR_DTTM_local, '34' as MAPPING_ID from adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) where (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL) and HEADER_EAR_DTTM_local &gt;= dateadd(d, -90, CAST(Getdate() AS hist_cust_po_date)) and MANIFEST_EAR_DTTM_local &gt;= dateadd(d, -90, CAST(Getdate() AS hist_cust_po_date));

SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, HEADER_EAR_DTTM_LOCAL, '49' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);

SELECT Distinct Descoped invoice_number&invc_itm_nbr, order_line_nbr,'26' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000')and  invc_type in ('zf2','zf2d','zf2f');

SELECT Distinct Descoped invoice_number&invc_itm_nbr, hist_cust_bus_chnl_nbr,'13' as MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) with (nolock) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') AND sold_by IN ('10', '20', '30', '40') and  invc_type in ('zf2','zf2d','zf2f') and hist_cust_bus_chnl_nbr is not null AND LTRIM(RTRIM(hist_cust_bus_chnl_nbr))!='';

SELECT Distinct Descoped invoice_number&invc_itm_nbr, order_type_code,'28' AS MAPPING_ID FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE Invc_Date &gt;= DATEADD(d, -365, CAST(Getdate() AS hist_cust_po_date)) AND sales_organization IN ('1000', '1200', '2000') and  invc_type in ('zf2','zf2d','zf2f');

SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, invoice_number, '41' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL) and invoice_number IS NOT NULL and ltrim(rtrim(invoice_number))!='';
...
, 'Customer_Name<>5K Account Number' as 'Additional_Column_Name'
, CONCAT(vendor_name_1,'<>',cust_corp_chain_nbr) as 'Additional_Column_Value'
, '604' AS MAPPING_ID FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd WITH (NOLOCK) 
...
select Distinct p.Descoped invoice_number&invc_itm_nbr as Primary_Key, CONCAT(invoice_number,invc_itm_nbr) as DQ_Column,'29' AS MAPPING_ID from adh_genpro_use2_prd.g_order360.billing_transactions_hhd p tablesample(10 percentage) WITH (NOLOCK) LEFT join adh_genpro_use2_prd.g_order360.billing_transactions_hhd r WITH (NOLOCK) on Descoped invoice_number&invc_itm_nbr = CONCAT(invoice_number,invc_itm_nbr) where  Invc_Date &gt;= dateadd(d, -365, CAST(Getdate() AS hist_cust_po_date)) and invc_date &gt;= dateadd(d, -365, CAST(Getdate() AS hist_cust_po_date)) and r.company_cd IN ('0061','0033') and p.invc_type IN ( 'zf2', 'zf2d', 'zf2f') and p.sales_organization IN ('1000','1200','2000') and p.Sold_By in ( '10', '20','30', '40');

SELECT 'KEY_BILL_NUM_BILL_ITEM' AS PRIMARY_KEY_NAME, Descoped invoice_number&invc_itm_nbr AS PRIMARY_KEY_VALUE, 'INVC_DATE' AS ATTRIBUTE_NAME, INVC_DATE AS ATTRIBUTE_VALUE, 'ORD_DATE' AS ADDITIONAL_COLUMN_NAME, created_date AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.g_order360.billing_transactions_hhd WHERE invc_type IN ('ZF2', 'ZF2F', 'ZF2D') AND SOLD_BY IN ('10', '20', '30', '40') AND sales_organization IN ('1000', '2000', '1200') AND created_date BETWEEN DATEADD(D, -180, GETDATE()) AND GETDATE();

select distinct 
ISNULL(LTRIM(RTRIM(E.external_license_number)),'0') AS external_license_number,
ISNULL(LTRIM(RTRIM(E.validity_end_date)),'0') AS validity_end_date,
'Customer City<>Customer Name<>Customer Number<>Customer Postal Code<>Customer State<>Customer Street<>EXT License number<>Legal Regulation<>Lecense Type<>License Valid End Date<>PLANT<>SAP License Number' AS ADDITIONAL_COLUMN_NAME,
CONCAT(ISNULL(LTRIM(RTRIM(K.sold_to_city)),'0'),'<>',ISNULL(LTRIM(RTRIM(K.vendor_name_1)),'0'),'<>',ISNULL(LTRIM(RTRIM(K.cust_nbr)),'0'),'<>',ISNULL(LTRIM(RTRIM(K.postal_cd)),'0'),'<>',ISNULL(LTRIM(RTRIM(K.region)),'0'),'<>',ISNULL(LTRIM(RTRIM(K.street)),'0')
,'<>',ISNULL(LTRIM(RTRIM(E.external_license_number)),'0'),'<>',ISNULL(LTRIM(RTRIM(E.legal_regulation_legal_control)),'0'),'<>',ISNULL(LTRIM(RTRIM(E.license_type)),'0'),'<>',ISNULL(LTRIM(RTRIM(E.validity_end_date)),'0'),'<>',coalesce(nullif(K.transp_zone,''),'0'),'<>',ISNULL(LTRIM(RTRIM(E.gen_mat_nbr_prepack)),'0')) AS ADDITIONAL_COLUMN_VALUE,
CASE 
WHEN DATEDIFF(D,GETDATE(),E.validity_end_date)<=30 AND DATEDIFF(D,GETDATE(),E.validity_end_date)>0 THEN 'Organization License Will Expire in next 30 days or less ' 
WHEN E.validity_end_date < GETDATE() THEN 'LICENSE EXPIRED' END AS 'INVALID_STATUS',
'1101' as Mapping_ID
from adh_genpro_use2_prd.s_customers.general_customer_hhd K WITH (NOLOCK)
 join adh_genpro_use2_prd.s_customers.partners_customers_license_hhd T WITH (NOLOCK)
on T.cust_nbr = K.cust_nbr
 join adh_genpro_use2_prd.s_customers.license_master_header_hhd E WITH (NOLOCK)
...
SELECT Distinct cust_nbr AS PRIMARY_KEY_1,sold_to_bus_prtnr_type_cd AS COLUMN_1,'1385' as Mapping_id FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd with(nolock);
...
select distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key,time_zone,'57' as MAPPING_ID from adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) with (nolock) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);
...
SELECT distinct CONCAT(consolidated_attrib,',',invoice_number,',',prod_id,',',acct_nbr) as Primary_Key, CO_CODE, '52' AS MAPPING_ID FROM adh_genpro_use2_prd.g_product360.ontime_delivery_hhd tablesample(10 percentage) WITH (NOLOCK) WHERE (consolidated_attrib IS NOT NULL OR invoice_number IS NOT NULL OR prod_id IS NOT NULL OR acct_nbr IS NOT NULL);
...
Select 'Customer Account Number' as 'PRIMARY_KEY_NAME', cust_nbr as 'PRIMARY KEY VALUE', 'Billing Plan Number' as 'ATTRIBUTE NAME', bill_plan_nbr as 'ATTRIBUTE VALUE', 'Customer Account Name<>Order Block<>Customer 5K Account Number' AS 'ADDITIONAL_COLUMN_NAME', concat(vendor_name_1,'<>',cust_central_ord_blk,'<>',cust_corp_chain_nbr) AS 'ADDITIONAL_COLUMN_VALUE' from adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd where cust_corp_chain_nbr in ('0500000153', '0500046319') AND cust_company_nbr not in ('0400004511', '0400002260') AND (cust_central_ord_blk IS NULL OR cust_central_ord_blk = ' ');
...
with DSD as (SELECT DISTINCT room_nbr, cust_nbr, deployment_stat, vendor_name_1 FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd WITH (NOLOCK)
...
SMT AS (SELECT DISTINCT room_nbr, cust_nbr, deployment_stat, vendor_name_1 FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd WITH (NOLOCK)
...
B340 AS (SELECT DISTINCT room_nbr, cust_nbr, deployment_stat, vendor_name_1 FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd WITH (NOLOCK)
...
HPOP AS (SELECT DISTINCT room_nbr, cust_nbr, deployment_stat, vendor_name_1 FROM adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd WITH (NOLOCK)
...
'Primary BU RG<>Primary BU ZF<>Primary BU ZG<>SAP Partner Function RG<>SAP Partner Function ZF<>SAP Partner Function ZG<>SAP Partner Number RG<>SAP Partner Number ZF<>SAP Partner Number ZG<>SAP Sold To Number'  AS ADDITIONAL_COLUMN_NAME,	
...
ISNULL(TRIM(knvp.sold_to_nbr),'0') as KNVPkunnr,
ISNULL(TRIM(knvp.PRTNR_FUNC),'0')  as knvpparvw,
ISNULL(TRIM(knvp.bus_prtnr_cust_account_number),'0')  as knvpKunn2,
ISNULL(TRIM(KNA.primary_business_unit),'0') as primary_business_unit
...
SELECT
    a.person_number,
    a.email_addr,
	'Customer Account Number<>Customer Account Name' AS ADDITIONAL_COLUMN_NAME,
...
'Customer Account Name<>Order Block<>Customer 5K Account Number' AS 'ADDITIONAL_COLUMN_NAME',
...
'Customer Account Name<>Order Block<>Customer 5K Account Number<>Created Date<>Created By' AS ADDITIONAL_COLUMN_NAME,
CONCAT(E.vendor_name_1,'<>',E.cust_central_ord_blk,'<>',E.cust_corp_chain_nbr,'<>',convert(varchar(30),K.created_date),'<>',K.created_by) AS ADDITIONAL_COLUMN_VALUE,
case
...
null as active_flg,
'1260' as Mapping_Id
from adh_genpro_use2_prd.s_customers.customer_details_consolidated_hhd E with (nolock) join adh_genpro_use2_prd.s_customers.general_customer_hhd K with (nolock) on E.cust_nbr = K.cust_nbr
...
SELECT CONVERT(VARCHAR(MAX), HashByteskey, 1) AS "PRIMARY_KEY_VALUE", VALIDITY_STATUS_RAW_SERV_QTY AS "ATTRIBUTE_VALUE", 'ORD_DT<>RAW_SERV_QTY<>ORD_QTY' AS "ADDITIONAL_COLUMN_NAME", CAST(ORD_DT AS VARCHAR) + '<>' + CAST(RAW_SERV_QTY AS VARCHAR) + '<>' + CAST(total_ord_qty AS VARCHAR) AS "ADDITIONAL_COLUMN_VALUE", '1651' AS MAPPING_ID FROM ( SELECT HASHBYTESKEY, total_ord_qty, RAW_SERV_QTY, ORD_DT, CASE WHEN RAW_SERV_QTY > total_ord_qty THEN 'INVALID' ELSE 'VALID' END AS VALIDITY_STATUS_RAW_SERV_QTY FROM data_type.data_type.data_type WHERE ORD_DT > DATEADD(cal_month, -6, GETDATE()) ) AS results;



SELECT CONVERT(VARCHAR(MAX), HashByteskey, 1) AS "PRIMARY_KEY_VALUE", VALIDITY_STATUS_RAW_SERV_LN_CT AS "ATTRIBUTE_VALUE", 'ORD_DT''<>''RAW_SERV_LN_CT' AS "ADDITIONAL_COLUMN_NAME", CAST(ORD_DT AS VARCHAR)+ '<>' +CAST(RAW_SERV_LN_CT AS VARCHAR) AS "ADDITIONAL_COLUMN_VALUE", '1652' AS MAPPING_ID FROM ( SELECT HASHBYTESKEY, RAW_SERV_LN_CT, ORD_DT, CASE WHEN RAW_SERV_LN_CT > ORD_LN_CT THEN 'INVALID' ELSE 'VALID' END AS VALIDITY_STATUS_RAW_SERV_LN_CT FROM data_type.data_type.data_type WHERE ORD_DT > DATEADD(cal_month, -6, GETDATE()) ) AS results;
...
select 'PCNUM' AS PRIMARY_KEY_NAME, pricing_contract_number AS PRIMARY_KEY_VALUE, 'VFDAT' AS ATTRIBUTE_NAME, valid_from_date AS ATTRIBUTE_VALUE, 'VTDAT' AS ADDITIONAL_COLUMN_NAME, valid_to_date AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_contracts.contract_header_hhd where valid_to_date>=dateadd(d,-180,getdate()) and valid_to_date is not null and valid_from_date IS NOT NULL and valid_from_date<>'' and valid_to_date<>'';

select 'PCNUM' AS PRIMARY_KEY_NAME, pricing_contract_number AS PRIMARY_KEY_VALUE, 'VTDAT' AS ATTRIBUTE_NAME, valid_to_date AS ATTRIBUTE_VALUE, 'VFDAT' AS ADDITIONAL_COLUMN_NAME, valid_from_date AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_contracts.contract_header_hhd where valid_to_date>=dateadd(d,-180,getdate());

SELECT 'Sold_To' + ' || ' + 'DATE' AS PRIMARY_KEY_NAME, sold_to_party + ' || ' + CAST([hist_cust_po_date] AS VARCHAR) AS PRIMARY_KEY_VALUE, '60D' AS ATTRIBUTE_NAME, [aging_60_days] AS ATTRIBUTE_VALUE, '[TOTAL_PD]'+ ' <> ' '[1-14D]' ' <> ' '[15-30D]' ' <> ' +'[31-60D]' AS ADDITIONAL_COLUMN_NAME, CONCAT_WS(' <> ',[total_payments_due],[aging_1_to_14_days],[aging_15_to_30_days],[aging_31_to_60_days]) AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_financials.enterprise_aging_hhd WHERE [hist_cust_po_date]>=dateadd(d,-180, GETDATE());

WITH CTE_COMBINED AS ( SELECT A.material_number, A.order_nbr, K.ord_line_nbr, K.sales_doc_nbr, A.service_ind, K.ZSERV_IND, CASE WHEN K.ZSERV_IND = 'Y' THEN 'VALID' ELSE 'INVALID' END AS VALIDITY_STATUS_SERVICELEVEL_IND FROM adh_genpro_use2_prd.g_order360.service_level_history_hhd A INNER JOIN data_type.data_type.data_type K ON A.material_number = K.ord_line_nbr AND A.order_nbr = K.sales_doc_nbr WHERE A.service_ind = 'Y' ) SELECT CAST(ord_line_nbr AS VARCHAR) + ' || ' + CAST(sales_doc_nbr AS VARCHAR) AS 'PRIMARY_KEY_VALUE', VALIDITY_STATUS_SERVICELEVEL_IND AS 'ATTRIBUTE_VALUE', 'ZSERV_IND' AS 'ADDITIONAL_COLUMN_NAME', ZSERV_IND AS 'ADDITIONAL_COLUMN_VALUE', '1666' as MAPPING_ID FROM CTE_COMBINED;
...
SELECT CONVERT(VARCHAR(MAX), HashByteskey, 1) AS "PRIMARY_KEY_VALUE", SERV_IND AS "ATTRIBUTE_VALUE", 'ORD_DT' AS "ADDITIONAL_COLUMN_NAME", ORD_DT AS "ADDITIONAL_COLUMN_VALUE", '1654' AS MAPPING_ID FROM data_type.data_type.data_type WHERE ORD_DT > DATEADD(DAY, -180, GETDATE());

select 'PCNUM' AS PRIMARY_KEY_NAME, pricing_contract_number AS PRIMARY_KEY_VALUE, 'PCNUM' AS ATTRIBUTE_NAME, pricing_contract_number AS ATTRIBUTE_VALUE, 'VTDAT' AS ADDITIONAL_COLUMN_NAME, valid_to_date AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_contracts.contract_header_hhd where valid_to_date>=dateadd(d,-180,getdate());



SELECT 'Sold_To' + ' || ' + 'DATE' AS PRIMARY_KEY_NAME, sold_to_party + ' || ' + CAST([hist_cust_po_date] AS VARCHAR) AS PRIMARY_KEY_VALUE, 'TOTAL_PD' AS ATTRIBUTE_NAME, [total_payments_due] AS ATTRIBUTE_VALUE, '[1-30D]'+ ' <> ' +'[31-60D]'+ ' <> ' +'[60D]' AS ADDITIONAL_COLUMN_NAME, CONCAT_WS(' <> ',[aging_1_to_30_days],[aging_31_to_60_days],[aging_60_days]) AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_financials.enterprise_aging_hhd where [hist_cust_po_date]>=dateadd(d,-180, GETDATE());

SELECT CONVERT(VARCHAR(MAX), HashByteskey, 1) AS 'PRIMARY_KEY_VALUE', ACCURACY_STATUS_MANF_LN_OUTS AS 'ATTRIBUTE_VALUE', 'TOT_LN_OUTS' + '<>' + 'WHSE_TMP_OUTS' + '<>' + 'original_value' + '<>' + 'calculated_value' AS 'ADDITIONAL_COLUMN_NAME', CONCAT_WS('<>', TOT_LN_OUTS, WHSE_TMP_OUTS,original_value,calculated_value) AS 'ADDITIONAL_COLUMN_VALUE', '1635' AS MAPPING_ID FROM ( SELECT HashByteskey,TOT_LN_OUTS,WHSE_TMP_OUTS,MANF_LN_OUTS, (TOT_LN_OUTS - WHSE_TMP_OUTS) AS calculated_value, MANF_LN_OUTS AS original_value, CASE WHEN (TOT_LN_OUTS - WHSE_TMP_OUTS) <> MANF_LN_OUTS THEN 'INACCURATE' ELSE 'ACCURATE' END AS ACCURACY_STATUS_MANF_LN_OUTS FROM data_type.data_type.data_type WHERE ORD_DT > DATEADD(cal_month, -6, GETDATE()) ) AS results;

SELECT CONVERT(VARCHAR(MAX), HashByteskey, 1) AS PRIMARY_KEY_VALUE, ACCURACY_STATUS_ADJSTD_LN_OUTS AS ATTRIBUTE_VALUE, 'ORD_LN_CT' + '<>' + 'WHSE_TMP_OUTS' + '<>' + 'original_value' + '<>' + 'calculated_value' AS ADDITIONAL_COLUMN_NAME, CONCAT_WS('<>', ORD_LN_CT, WHSE_TMP_OUTS, original_value, calculated_value) AS ADDITIONAL_COLUMN_VALUE, '1636' AS MAPPING_ID FROM ( SELECT HashByteskey, ORD_LN_CT, WHSE_TMP_OUTS, ADJSTD_LN_OUTS, (ORD_LN_CT - WHSE_TMP_OUTS) AS calculated_value, (ADJSTD_LN_OUTS) AS original_value, CASE WHEN (ORD_LN_CT - WHSE_TMP_OUTS) <> ADJSTD_LN_OUTS THEN 'INACCURATE' ELSE 'ACCURATE' END AS ACCURACY_STATUS_ADJSTD_LN_OUTS FROM data_type.data_type.data_type WHERE ORD_DT > DATEADD(cal_month, -6, GETDATE()) ) AS results;

SELECT CONVERT(VARCHAR(MAX), HashByteskey, 1) AS 'PRIMARY_KEY_VALUE', ACCURACY_STATUS_ADJSTD_LN_72INC_CT AS 'ATTRIBUTE_VALUE', 'ADJSTD_LN_OUTS' + '<>' + 'TRU_TMP_OUTS_EXCP' + '<>' + 'original_value' + '<>' + 'calculated_value' AS 'ADDITIONAL_COLUMN_NAME', CONCAT_WS('<>', ADJSTD_LN_OUTS, TRU_TMP_OUTS_EXCP,original_value,calculated_value) AS 'ADDITIONAL_COLUMN_VALUE', '1637' AS MAPPING_ID FROM ( SELECT HashByteskey,ADJSTD_LN_OUTS,TRU_TMP_OUTS_EXCP, (ADJSTD_LN_OUTS + TRU_TMP_OUTS_EXCP) AS calculated_value, ADJSTD_LN_72INC_CT AS original_value, CASE WHEN ((ADJSTD_LN_OUTS) + TRU_TMP_OUTS_EXCP) <> ADJSTD_LN_72INC_CT THEN 'INACCURATE' ELSE 'ACCURATE' END AS ACCURACY_STATUS_ADJSTD_LN_72INC_CT FROM data_type.data_type.data_type WHERE ORD_DT > DATEADD(cal_month, -6, GETDATE()) ) AS results;

WITH CTE1 AS ( SELECT col_header as per mapping, delivery_item_number, delivery_number, COUNT(*) AS DUPLICATE_COUNT FROM adh_genpro_use2_prd.g_order360.abops_timetofill_hhd GROUP BY col_header as per mapping, delivery_item_number, delivery_number ) SELECT 'DOC_NUMBER_ITEM_KEY' + ' || ' + 'DELIV_ITEM' + ' || ' + 'DELIV_NUMB' AS 'PRIMARY_KEY_NAME', CAST(col_header as per mapping AS VARCHAR) + ' || ' + CAST(delivery_item_number AS VARCHAR) + ' || ' + CAST(delivery_number AS VARCHAR) AS 'PRIMARY_KEY_VALUE', 'DOC_NUMBER_ITEM_KEY' + ' || ' + 'DELIV_ITEM' + ' || ' + 'DELIV_NUMB' AS 'ATTRIBUTE_NAME', CAST(col_header as per mapping AS VARCHAR) + ' || ' + CAST(delivery_item_number AS VARCHAR) + ' || ' + CAST(delivery_number AS VARCHAR) AS 'ATTRIBUTE_VALUE', 'DUPLICATE_COUNT' AS 'ADDITIONAL_COLUMN_NAME', DUPLICATE_COUNT AS 'ADDITIONAL_COLUMN_VALUE' FROM CTE1;



select 'PCNUM' AS PRIMARY_KEY_NAME, pricing_contract_number AS PRIMARY_KEY_VALUE, 'VFDAT' AS ATTRIBUTE_NAME, valid_from_date AS ATTRIBUTE_VALUE, 'VTDAT' AS ADDITIONAL_COLUMN_NAME, valid_to_date AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_contracts.contract_header_hhd where valid_to_date>=dateadd(d,-180,getdate()) and valid_to_date is not null and valid_from_date IS NOT NULL and valid_from_date<>'' and valid_to_date<>'';

SELECT 'Sold_To' + ' || ' + 'DATE' AS PRIMARY_KEY_NAME, sold_to_party + ' || ' + CAST([hist_cust_po_date] AS VARCHAR) AS PRIMARY_KEY_VALUE, 'OPEN_AR' AS ATTRIBUTE_NAME, open_accounts_receivable AS ATTRIBUTE_VALUE, 'CURRENT'+ '<>' +'TOTAL_PD' AS ADDITIONAL_COLUMN_NAME, CONCAT_WS('<>',[Current_due_amt],[total_payments_due]) AS ADDITIONAL_COLUMN_VALUE FROM adh_genpro_use2_prd.s_financials.enterprise_aging_hhd WHERE [hist_cust_po_date]>=dateadd(d,-180, GETDATE());
